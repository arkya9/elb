...
export const login = async (req, res) => {
  ...
  const checkUsername = await pool.query(
    `select count(*) from user_master where email=$1 and is_active=true`,
    [username]
  );
  ...
  const user = await pool.query(
    `select * from user_master where email=$1 and is_active=true`,
    [username]
  );
...
export const currentUser = async (req, res) => {
  ...
  const user = await pool.query(
    `select user_master.*, role_master.role from user_master join role_master on user_master.role_id = role_master.id where user_master.uuid=$1`,
    [uuid]
  );
...
export const resetPassword = async (req, res) => {
  ...
  try {
    ...
    await pool.query(`update user_master set password=$1 where email=$2`, [
      newPassword,
