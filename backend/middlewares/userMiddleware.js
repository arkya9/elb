...
export const validateUser = withValidationErrors([
  ...
  body("email")
    ...
    .custom(async (value, { req }) => {
      ...
      const check = await pool.query(
        `select count(*) from user_master where email=$1 ${append}`,
        values
      );
  ...
  body("mobile")
    ...
    .custom(async (value, { req }) => {
      ...
      const values = userId ? [value, userId] : [value];
      const check = await pool.query(
        `select count(*) from user_master where mobile=$1 ${append}`,
        values
      );
      if (Number(check.rows[0].count) > 0) {
  ...
  body("roleId")
    ...
    .custom(async (value) => {
      if (value) {
        ...
        if (Number(check.rows[0].count) === 0) {
          throw new BadRequestError(`Don't try to fuck with the system Dude!!`);
        }
        return true;
      }
    }),
