...
export const mindUrOwnPage = async (req, res, next) => {
  ...
  const user = await pool.query(`select slug from user_master where uuid=$1`, [
    uuid,
  ]);
...
export const validateRegister = withValidationErrors([
  ...
  body("email")
    ...
    .custom(async (value) => {
      const check = await pool.query(
        `select count(*) from user_master where email=$1`,
        [value]
      );
  ...
  body("mobile")
    ...
    .custom(async (value) => {
      const check = await pool.query(
        `select count(*) from user_master where mobile=$1`,
        [value]
      );
      if (Number(check.rows[0].count) > 0) {
...
export const validateForgotPass = withValidationErrors([
  body("email")
    ...
    .custom(async (value) => {
      const check = await pool.query(
        `select count(id) from user_master where email=$1 and is_active=true`,
        [value]
      );
